...
if ($mysqli->connect_errno) {
	...
	//These are the questions you want graphs for
	//get only the data for these questions and make the graphs
	
	
	echo $mysqli->host_info . "\n";
	
	//get all the question ids we want graphs for
	$allQuestions = $mysqli->query("SELECT qid,type, quesType FROM questions");

	foreach ($allQuestions as $key => $qid) {
		$quesId =  $qid[qid];
		$quesType =  $qid[quesType];
		$chartType =  $qid[type];

		if ($quesType == "MCQ") {
			$resMcq = $mysqli->query("SELECT options,count FROM mcq where qid='$qid[qid]'");

			foreach ($resMcq as $key => $value) {
				//print_r($value);
			
			if ($chartType == "Bar") {
				# code...
				//send the values to bar chart function
				echo $value[options];
				echo $value[count];
			} elseif ($chartType == "Bubble") {
				# code...
				//send the values to bubble function
				echo $value[options];
				echo $value[count];
			}
		}
			...
			$resFreeText = $mysqli->query("SELECT qid,content FROM freetext where qid='$qid[qid]'");
			foreach ($resFreeText as $key => $value) {
				//echo $value[content];
			}
		}
